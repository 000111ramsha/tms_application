...
class FormSubmissionService {
  ...
  static async submitBDIAssessment(formData) {
    try {
      ...
      // Generate and send PDF with original form data
      await PDFEmailService.generateAndSendPDF('BDI', formData);
  ...
  static async submitPHQ9Screening(formData) {
    try {
      ...
      // Generate and send PDF with original form data
      await PDFEmailService.generateAndSendPDF('PHQ-9', formData);
  ...
  static async submitMedicalHistory(formData) {
    try {
      ...
      console.log('Submitting medical history data:', medicalHistoryData); // Add logging
      ...
      // Generate and send PDF with original form data
      await PDFEmailService.generateAndSendPDF('Medical History', formData);
  ...
  static async submitPreCertMedList(formData) {
    try {
      ...
      // Convert temporary session ID to UUID
      ...
      const patientSessionId = sessionData.id;

      // Create base form submission object
      ...
      Object.entries(formData.medications).forEach(([category, medications]) => {
        Object.entries(medications).forEach(([medName, details]) => {
          if (details.selected) {
            ...
            // Create medication details object
            const medicationDetails = {
              selected: true,
              dose: details.dosage || null,
              start_date: details.startDate || null,
              end_date: details.endDate || null,
              reason_discontinuing: details.reasonForDiscontinuing || null
            };

            // Add to form submission
            formSubmission[baseColumnName] = medicationDetails;
          }
        });
      });
  ...
  static async submitPatientDemographics(formData) {
    try {
      ...
      // Generate and send PDF with original form data
      await PDFEmailService.generateAndSendPDF('Patient Demographics', formData);

      return { success: true, data: patientResult };
    } catch (error) {
